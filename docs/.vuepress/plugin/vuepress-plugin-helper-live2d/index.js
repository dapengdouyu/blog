...
module.exports = (options = {}, context) => ({
  ...
  async ready() {
    ...
    const live2dConfig =
      options.live2d ||
      (siteConfig.themeConfig.yilia_plus
        ? siteConfig.themeConfig.yilia_plus.live2d
        : {});
    // 合并默认配置和插件配置
    options.live2d = _.defaultsDeep(
      {},
      live2dConfig,
      defaultPluginConfig.live2d
    );

    // 进行删除文件夹(查找需要删除的文件夹名称之后才进行删除，不建议全删除整个文件夹)
    const delDirPath = path.join(
      sourceDir || options.dest,
      options.outDir,
      options.outDirName
    );
    const delFolders = fileUtil.listFolder(
      path.resolve(__dirname, path.join("./public/live2d"))
    );
    ...
    if (!!delFolders) {
      for (let folder of delFolders) {
        ...
        if (
          options.live2d &&
          options.live2d.enable &&
          modelDirName == options.live2d.model
        ) {
          // 正在使用的则跳过删除
          options.log &&
            helper_live2d_log(`跳过删除--- ${options.live2d.model}`);
          continue;
        }
        // 待删除的模型文件夹
    ...
    if (options.live2d && options.live2d.enable) {
      ...
      const modelFiles = fileUtil.listFiles(
        path.resolve(
          __dirname,
          path.join("./public/live2d", options.live2d.model)
        )
      );
      console.log(
        "------>",
      ...
      if (!!modelFiles) {
        ...
        for (const file of modelFiles) {
          ...
          const tempOutFilePath = path.relative(
            path.resolve(__dirname, path.join("./public/live2d")),
            file
          );
...
${content}\n\n`.trim()
  ...
  define() {
    if (options.log) {
      helper_live2d_log(
        "加载插件配置：" +
          (JSON.stringify ? JSON.stringify(options.live2d) : options.live2d)
      );
    }
    return {
